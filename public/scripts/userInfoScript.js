...
async function getUserPhone() {
    var user_inf = document.getElementById('student-inf.phone');
    if (user_inf.innerHTML.length > 0) return;
    await fetch("http://127.0.0.1:3000/users/profile", {
        method: "GET",
        headers: {
...
async function getUserDate() {
    var user_inf = document.getElementById('student-inf.birthday');
    if (user_inf.innerHTML.length > 0) return;
    await fetch("http://127.0.0.1:3000/users/profile", {
        method: "GET",
        headers: {
        ...
        // Parse the date and format it (DD-MM-YYYY)
        ...
        user_inf.innerHTML = formattedDate;
    }).catch((err) => {
        console.log(err);
    });
};
